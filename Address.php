...
/**
  * ttestestAddresses class, Address.php
...
class		Address extends ObjectModel
{
	...
	//joseph bof. 
		public static function getZonesById($id_address)
		{
			
			$result = Db::getInstance()->ExecuteS('
			SELECT c.`id_zone`
			FROM `'._DB_PREFIX_.'address` a
			LEFT JOIN `'._DB_PREFIX_.'country_carrier_zone` c ON c.`id_country` = a.`id_country`
			LEFT JOIN `'._DB_PREFIX_.'state` s ON s.`id_state` = a.`id_state`
			WHERE a.`id_address` = '.intval($id_address).'
			GROUP BY c.`id_zone`
			LIMIT 0 , 30');
