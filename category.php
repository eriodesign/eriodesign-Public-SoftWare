...
else
{
	...
	else
	{
		...
		//edited by cyf on 2012-04-06
		global $cookie;
		$newProductNbr = 8;
		$bestSellersNbr = 8;

		//new products in category
		$newProducts = Product::getNewProductsByCategory(intval($cookie->id_lang), 0, intval($newProductNbr), '', '', '', $category->id);

		//bestsellers in category
		$bestsellers = ProductSale::getBestSalesLightByCategory(intval($cookie->id_lang), 0, $bestSellersNbr, $category->id);

		$best_sellers = array();
		foreach ($bestsellers AS $bestseller)
		{
			$bestseller['price'] = Product::getPriceStatic(intval($bestseller['id_product']));
			$best_sellers[] = $bestseller;
		}

		echo "<pre>";
		//echo "a";print_r($newProducts);
		//echo "b";print_r($best_sellers);
		echo "</pre>";
		$smarty->assign(array(
			'products' => (isset($cat_products) AND $cat_products) ? $cat_products : NULL,
			'newproducts' => $newProducts ? $newProducts : NULL,
			'bestsellerproducts' => $best_sellers ? $best_sellers : NULL,
