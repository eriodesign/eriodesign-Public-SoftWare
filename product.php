...
//beging of clean my textFields cookie to reset	py by:let
$clean = isset($_GET['clean']) ? intval($_GET['clean']) : 0;
if ($clean == 1){
		$cookie->unsetFamily('textFields_'.intval($_GET['id_product']));//delete custom
		
		$product = new Product(intval($_GET['id_product']), true, intval($cookie->id_lang));
		$link = new Link(intval($_GET['id_product']));
		$rewrited_url = $link->getProductLink($product->id, $product->link_rewrite);
		header('location:'.$rewrited_url);
		exit();											   
}
//end of clean my textFields cookie to reset
...
$css_files = array(
	...
	_THEME_CSS_DIR_.'productCustomization.css' => 'screen',
);
...
$js_files = array(
	...
	_THEME_JS_DIR_.'productCustomization.js'
);
...
else
{
	...
	else
	{
		...
		//bof of check is or not custom	let
		count($cookie->getFamily('textFields_'.intval($product->id))) > 0 ? $smarty->assign(array('my_custom' => 1)) : $smarty->assign(array('my_custom' => 0));
		//eof of check is or not custom	let
...
foreach($groups as $key => $value)
	$Custom_groups[] = $key;//color && size

$Custom_groups[0] = $Custom_groups[1];//color
$Custom_groups[1] = $Custom_groups[2];//size
?>
<style type="text/css">
...
.msgbox_content {padding:10px;float:left;line-height: 20px;background:#fafafa;text-indent:7px;width:490px;}
.msgbox_icon {width: 50px;height: 50px;float: left;text-align: center;line-height:50px;padding-top:10px;display:none;}
.msgbox_mask { background-color:#000000;height:100%;left:0;opacity:0.4;filter:alpha(opacity=40);position:absolute;top:0;width:100%;z-index:99998;}
.msgbox {background-color:#EFFAFE;border:5px solid #EEEEEE;font-size:12px;height:auto;left:450px;position:absolute;top:320px;z-index:99999;}

#product #msgBoxContent .textField13{border-top:none;display:block;margin-right:0;margin-top:0;padding:0;padding-bottom:3px;padding-left:0;padding-right:0;padding-top:3px;text-indent:7px;width:465px;}
#product .name_customization{display:block;float:left;text-align:right;width:130px;font-family:"Lucida Sans Unicode","Lucida Grande","Trebuchet MS",sans-serif,Helvetica,Sans-Serif;
font-size:12px;}
#product li.customizationUploadLine {background:#F8F8F8 none repeat scroll 0 0;border:1px solid #EEEEEE;display:block;float:left;margin:2px 15px 2px 0;padding:3px;width:223px;}


#product #msgBoxContent .textField1, #product #msgBoxContent .textField3, #product #msgBoxContent .textField5, #product #msgBoxContent .textField7, #product #msgBoxContent .textField9, #product #msgBoxContent .textField11 {
float:right;
margin-right:0;
}


#product .customization_block_input{width:36px;}
#customizationForm p{font-size:11px;}
</style>
<script language="javascript">
var Color = 'group_2';//Color's group id
var Size = 'group_4';//Size's group id
var Timer = null;
var OpacityValue = 0;
var ScrollTop = 0;
var ScrollLeft = 0;
...
{
	var Units = '';
	obj.value != 'IN' ? Units = 'CM' : Units = 'IN';
	for(i=0; i < <?php echo (count($product->getCustomizationFields((int)($cookie->id_lang))) );?>; i++)	document.getElementById('Units'+i).innerHTML = Units;
	
}

function Add_value()
{
	for(i=0; i < <?php echo (count($product->getCustomizationFields((int)($cookie->id_lang))));?>; i++){
		if (document.getElementById('textField'+i).value != ''){
			document.getElementById('textField'+i).value = document.getElementById('textField'+i).value + document.getElementById('Units'+i).innerHTML;	
		}	
	}
...
function My_clear()
{
	obj = document.getElementById('select_1');
	for(i=0; i < <?php echo (count($product->getCustomizationFields((int)($cookie->id_lang))));?>; i++){
		if (document.getElementById('textField'+i).value.indexOf('CM')>0){
			document.getElementById('textField'+i).value = document.getElementById('textField'+i).value.replace(/CM/g,'');
			obj = document.getElementById('select_1');
		}else{
			document.getElementById('textField'+i).value = document.getElementById('textField'+i).value.replace(/IN/g,'');
			obj = document.getElementById('select_2');
		}
	}
	try{
		obj.checked = true;
		...
		if (document.getElementById('Units0').value.indexOf('CM')>0){
			obj = document.getElementById('select_1');
			obj.checked = true;
			Units(obj);
		}else{
